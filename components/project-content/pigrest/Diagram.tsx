  ...
  return (
    <>
      <section id="architecture" className="mb-6">
        ...
        ※ 게시물 기능은 사용자가 작성중인 게시물을 임시로 저장하는{" "}
        ...
        <div className="mt-4">
          <p className="mb-2">사용자 입력 시 발생하는 DB 부하를 줄이기 위해 <b>Write Behind</b> 전략을 적용한 자동 저장 시스템을 구현하였습니다.</p>
          <p className="mb-2">
            입력한 데이터는 <b>Redis Hash</b>에 임시 저장(draft)되며, Spring Scheduler를 통해 <b>주기적으로 DB에 비동기 반영</b>됩니다.
            초기{" "}{"`draft:*`"} 패턴으로 전체 key를 스캔하는 방식에서 최신 업데이트된 데이터의 key를 관리하는 Set 기반 방식으로 개선하여,
            <b> 중복 동기화를 방지</b>하고 시간 복잡도를 O(N)에서 O(1)로 최적화했습니다.
            또한 <b>Redis 파이프라인</b>을 적용하여 IO 성능을 개선함으로써 1000개의 요청을 기준으로 약 1450ms에서 264ms로 단축했습니다.
          </p>
          <p className="mb-2">
          ...
          <p className="mb-2">
            게시물 발행(publish) 시 Unique Constraint를 적용하여 <b>중복 요청을 방지</b>하여 멱등성을 보장했습니다.
            그와 동시에 Redis에 저장된 데이터를 즉시 삭제하여 메모리 효율성 또한 높였습니다.
          </p>
        </div>
      ...
      <section id="uuid" className="mb-6">
        ...
        <div>
          <p className="mb-2">
            Auto Increment 기반 ID의 보안 취약점을 해결하기 위해 UUID v7을 도입하였습니다.
          </p>
          <p className="mb-2">
            UUID를 BINARY(16) 형태로 저장하여 1억개 데이터 기준 약 2GB의 저장 공간을 절약했으며,
            시간 기반 UUID v7로 B-Tree 인덱스 성능을 최적화하여 쿼리 효율성을 개선했습니다.
          </p>
        </div>
        <div className="mt-4 flex gap-3 border-l-4 border-[#61afef] bg-[#61afef]/10 rounded-r px-4 py-3 text-sm">
          <BookOpen size={16} className="text-[#61afef] flex-shrink-0 mt-0.5" />
      ...
      <section id="ci-cd-pipeline" className="mb-6">
        ...
        <div>
          ...
          <p className="mb-2">
            ...
            이후, EC2에서 Docker Compose로 MySQL, Redis, 애플리케이션을 오케스트레이션하며
            Infrastructure as Code(IaC) 방식을 적용하여 선언적이고 안정적인 배포 환경을 구성했습니다.
          </p>
        </div>
      </section>
    </>
